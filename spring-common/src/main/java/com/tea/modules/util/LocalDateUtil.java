...
public class LocalDateUtil {
    ...
    public static List<String> getDateListByStartAndEnd(LocalDate startDate, LocalDate endDate) {
        List<LocalDate> weekDateList = LocalDateUtil.getWeekDateList(startDate, endDate);
        List<String> key = new ArrayList<>();
        weekDateList.forEach(date -> key.add(LocalDateUtil.getStartAndEndOfWeekByDate(date)));
        return key;
    }
    ...
    public static Map<String,Object> getWeekResultMap(List<Map<String, Object>> resultMapFromMySQL, LocalDate startDate, LocalDate endDate){
        ...
        resultMapFromMySQL.forEach(map -> {
            ...
            for (int i = 0; i < weekDateList.size(); i++) {
                ...
                if (LocalDateUtil.isSameWeek(localDate, weekStamp)) {
    ...
    public static LocalDateTime stringToLocalDateTime(String stringValue) {
        LocalDateTime localDateTime;
        try {
            localDateTime = LocalDateTime.parse(stringValue, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        } catch (DateTimeParseException e) {
            throw new RuntimeException("日期转化错误，请查看日期格式是否正确!");
        }
